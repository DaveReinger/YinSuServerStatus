# ServerStatus中文版(魔改版)：   
# 本项目为私人使用，基本不提供使用文档
# [原始项目地址 cppla/ServerStatus](https://github.com/cppla/ServerStatus)    
...
【服务端】
```bash
docker run -d --restart=always --name=serverstatus -v ~/ServerStatus/config.json:/ServerStatus/server/config.json -v ~/ServerStatus/html:/usr/share/nginx/html  -p 8080:80 -p 35601:35601 cppla/serverstatus
```
【客户端】
```bash
wget --no-check-certificate -qO client-linux.py 'https://raw.githubusercontent.com/DaveReinger/YinSuServerStatus/master/clients/client-linux.py' && nohup python client-linux.py SERVER=45.79.67.132 USER=s04  >/dev/null 2>&1 &
```
【相关配置文件】
```json
{"servers":
...
wget https://raw.githubusercontent.com/DaveReinger/YinSuServerStatus/master/autodeploy/config.json
